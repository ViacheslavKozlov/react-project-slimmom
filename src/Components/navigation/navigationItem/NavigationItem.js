...
const NavigationItem = ({ name, path, exact, toggleModal, isAuth }) => {
  ...
  return (
    <li className={isAuth && (isMobileDevice || isTabletDevice) ? styles.privateItem : styles.item}>
      {name !== "Home" ? (
        <NavLink
          onClick={toggleModal}
          ...
          activeClassName={!isDescDevice && isAuth ? styles.activeLinkPrivate : styles.activeLink}
        >
          {name}
        </NavLink>
      ) : (
        <NavLink to={path} exact={exact} className={styles.link} activeClassName={styles.activeLink}>
          {isMobileDevice ? (
            <>
              {!isAuth ? (
                <img src={mobileLogoWithoutSlim} alt="Logo" className={styles.logo_without_slim} />
              ) : (
