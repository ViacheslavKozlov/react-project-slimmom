...
const AuthPage = () => {
  ...
  const handleSubmit = userData => {
    pathname === mainRoutes[4].path ? dispatch(authRegistration(userData)) : dispatch(authLogin(userData));
  };
  ...
  useEffect(
    () => {
      ...
      if (pathname === "/register" && userId.id !== "" && error === "") {
        const isComing = window.confirm("Вы успешно зарегистрировались, перейти на страницу входа?");
  ...
  return mainRoutes
    ...
    .map(({ path, name }) => path === pathname && <AuthTempForm handleSubmit={handleSubmit} btnName={name} key={path} />);
};
